...
function App() {
  ...
  const handleAdminAccess = async (e: React.FormEvent) => {
    ...
    if (!adminUsername || !adminPassword) {
      setAuthError('Identifiant et mot de passe requis');
      return;
    }

    setIsLoading(true);
    wsServiceInstance.sendAuthentication('admin_access', adminUsername, adminPassword);
    setAdminUsername('');
    setAdminPassword('');
  };
      ...
      <div className="fixed inset-0 bg-black/95 backdrop-blur-xl flex items-center justify-center p-6 z-50">
        <div className="bg-slate-900/90 backdrop-blur-xl border border-slate-700/50 rounded-3xl p-8 shadow-2xl max-w-md w-full">
          <div className="text-center mb-8">
            ...
            <p className="text-slate-400 text-lg">Connectez-vous avec vos identifiants</p>
          </div>
